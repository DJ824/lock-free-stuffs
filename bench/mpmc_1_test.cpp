...
void test_try_api_and_fifo_order() {
    LockFreeQueueMpmc1<std::uint64_t, 8> queue;
...
void test_first_blocking_emplace_on_empty_queue() {
    ...
    if (!producer_done.load(std::memory_order_acquire)) {
        std::cerr << "timeout: mpmc_1 producer failed to enqueue into an empty queue\n";
        std::abort();
    }
...
void test_mpmc_stress() {
    ...
    LockFreeQueueMpmc1<std::uint64_t, 1024> queue;
