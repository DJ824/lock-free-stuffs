...
#pragma GCC diagnostic pop
    ...
    template <typename T>
    concept copyable = std::is_trivially_copyable_v<T> && (sizeof(T) <= 16);
    // Copy types that can be passed in two registers, otherwise move

    template <typename T>
    concept copyable_or_nothrow_move_assignable = copyable<T> || std::is_nothrow_move_assignable_v<T>;
    // Copy types that can be passed in two registers, otherwise move
    ...
    */
    template <typename T, size_t SIZE, bool MINIMISE_LATENCY = true, bool NONBLOCKING = true, size_t PAUSE_SHORT = 3,
              size_t PAUSE_LONG = 40>
    ...
    public:
        ...
        size_t was_size() noexcept {
            return std::clamp(
                static_cast<size_t>(_back.load(std::memory_order_relaxed) - _front.load(std::memory_order_relaxed)),
                static_cast<size_t>(0), SIZE);
        }
        ...
        */
        ...
        ORBIT_FORCE_INLINE bool do_push(std::conditional_t<copyable<T>, T, T&&> element) noexcept {
            while (true) {
                ...
                uint64_t expected_state = ((cycle - 1) << 2) | slot_state::EMPTY;
                // Expect empty slot from last cycle (most likely)
                ...
                if ((!_states[(prev_back * STEP) & MODULO_MASK].compare_exchange_weak(
                    ...
                    if constexpr (return_if_full || !MINIMISE_LATENCY) {
                        if ((expected_state >> 2) == (cycle - 1)) [[unlikely]] {
                            ...
                            // No need for a very long pause here even when optimising for throughput
                    ...
                    if constexpr (NONBLOCKING) {
                        if (expected_state == desired_state)
                        // Check if another thread has already started writing here (only possible if multiple producers)
                        {
                            _back.compare_exchange_weak(prev_back, prev_back + 1, std::memory_order_relaxed,
                                                        std::memory_order_relaxed);
                            // Help move the back if no one else does
                        }
                    }
                    continue;
                }
                ...
                if constexpr (NONBLOCKING) {
                    ...
                    _back.compare_exchange_weak(prev_back_copy, prev_back + 1, std::memory_order_relaxed,
                                                std::memory_order_relaxed); // Update back if nobody else did
                }
                else {
                ...
                _states[(prev_back * STEP) & MODULO_MASK].store((cycle << 2) | slot_state::FULL,
                                                                std::memory_order_release);
                // Ensure element is written before this store takes place
        ...
        ORBIT_FORCE_INLINE bool do_pop(T& result) noexcept {
            while (true) {
                ...
                uint64_t expected_state = (cycle << 2) | slot_state::FULL;
                // Expect full slot from this cycle (most likely)
                ...
                if ((!_states[(prev_front * STEP) & MODULO_MASK].compare_exchange_weak(
                    expected_state, desired_state, std::memory_order_acquire, std::memory_order_relaxed))) {
                    ...
                    if constexpr (NONBLOCKING) {
                        ...
                        {
                            _front.compare_exchange_weak(prev_front, prev_front + 1, std::memory_order_relaxed,
                                                         std::memory_order_relaxed);
                            // Help move the front if no one else does
                        }
                    }
                    continue;
                }
                ...
                if constexpr (NONBLOCKING) {
                    ...
                    _front.compare_exchange_weak(prev_front_copy, prev_front + 1, std::memory_order_relaxed,
                                                 std::memory_order_relaxed); // Update front if nobody else did
                }
                else {
                ...
                _states[(prev_front * STEP) & MODULO_MASK].store((cycle << 2) | slot_state::EMPTY,
                                                                 std::memory_order_release);
                // Ensure element is read before this store takes place
                return true;
            }
        };
        ...
        static constexpr uint64_t MODULO_MASK = SIZE - 1;
        // Given N, N & MODULO_MASK is equivalent to (and faster than) N % SIZE
